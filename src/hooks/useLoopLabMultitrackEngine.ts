...
export type LoopLabUiState = {
  status: 'off' | 'preparing' | 'active' | 'error';
  region: LoopLabRegionState | null;
  error: string | null;
};
...
export function useLoopLabMultitrackEngine(
  ...
  const [loopState, setLoopState] = useState<LoopLabUiState>({
    status: 'off',
    region: null,
    error: null,
  });
  ...
  const getEngine = useCallback((targetKind: EngineKind) => {
    ...
    if (typeof window === 'undefined') {
      throw new Error('useLoopLabMultitrackEngine requires a browser environment.');
    }
  ...
  const initialize = useCallback(async (tracks: TrackData[]) => {
    ...
    setLoopState({ status: 'off', region: null, error: null });
    ...
    try {
      ...
      if (targetKind === 'streaming') {
        ...
        console.warn(
          '[useLoopLabMultitrackEngine] Streaming engine failed during initialization. Switching to the compatible media/buffer engine.',
          error,
        );
        ...
        try {
          ...
          console.error(
            '[useLoopLabMultitrackEngine] Compatible media/buffer fallback also failed.',
            fallbackError,
          );
  ...
  const configureLoopRegion = useCallback(async (
    ...
    if (!(engine instanceof StreamingMultitrackEngine)) {
      setLoopState({
        status: 'error',
        region: null,
        error: 'El laboratorio de bucle requiere el motor streaming aislado.',
      });
      throw new Error('El laboratorio de bucle requiere el motor streaming aislado.');
    }

    setLoopState((previous) => ({
      status: enabled ? 'preparing' : 'off',
      region: enabled ? previous.region : null,
      error: null,
    }));

    try {
      const region = await engine.configureLoopRegion(startInSeconds, endInSeconds, enabled);
      setLoopState({
        status: region ? 'active' : 'off',
        region,
        error: null,
      });
      commitCurrentTime(engine.getCurrentTime());
      ...
      const message = error instanceof Error ? error.message : 'No se pudo preparar el bucle.';
      setLoopState({ status: 'error', region: null, error: message });
      commitCurrentTime(engine.getCurrentTime());
      setIsPlaying(engine.getIsPlaying());
      throw error;
    }
