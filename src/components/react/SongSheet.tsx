...
type RenderableChord = GroupedChord & {
...
const buildRenderableChords = (line: ParsedChordLine): RenderableChord[] => {
...
const buildChordGuide = (line: ParsedChordLine): string => {
  ...
  const totalLength = groupedChords.reduce((maxLength, item) => (
    Math.max(maxLength, item.renderPosition + item.text.length)
  ), lyrics.length);
