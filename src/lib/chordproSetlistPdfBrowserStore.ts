...
const STORAGE_PREFIX = 'alabanza:chordpro-setlist-pdf:';
...
export const createChordProSetlistPdfBrowserToken = (
  payload: ChordProSetlistPdfPayload
) => {
  const storage = getStorage();
  if (!storage) {
    throw new Error('Este navegador no permite preparar el setlist para imprimir.');
  }
...
export const readChordProSetlistPdfBrowserToken = (token: string) => {
  const storage = getStorage();
  ...
  try {
    ...
    return normalizeChordProSetlistPdfPayload(parsed?.payload);
  } catch {
    return null;
  }
};

export const deleteChordProSetlistPdfBrowserToken = (token: string) => {
  const storage = getStorage();
