...
const DEFAULT_WORKLET_MODULE_URL = `/workers/LoopLabMultitrackWorkletProcessor.js?v=${AUDIO_WORKER_ASSET_VERSION}`;
const DEFAULT_PRODUCER_WORKER_URL = `/workers/LoopLabAudioProducerWorker.js?v=${AUDIO_WORKER_ASSET_VERSION}`;
    ...
    try {
      while (!trackState.endOfStreamReached && !trackState.abortController.signal.aborted) {
        ...
        const response = await fetch(trackState.config.url, {
          mode: 'cors',
          credentials: new URL(trackState.config.url, window.location.href).origin === window.location.origin
            ? 'same-origin'
            : 'omit',
          headers: {
