...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    await protectPdfGenerationRequest({ request, cookies, bucket: 'pdf-chordpro-v2' });
    ...
    page.on('console', (message) => {
      console.log(`[ChordPro PDF V2][page console][${message.type()}] ${message.text()}`);
    });
    page.on('pageerror', (error) => {
      console.error('[ChordPro PDF V2][page error]', error);
    });
    page.on('requestfailed', (failedRequest) => {
      console.warn(
        `[ChordPro PDF V2][request failed] ${failedRequest.failure()?.errorText || 'unknown'} ${
          failedRequest.url()
        }`
      );
    });
    ...
    console.error(`ChordPro PDF V2 generation failed at stage "${failureStage}":`, error);
