...
class MultitrackWorkletProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.loopCacheByTrackIndex = new Map();
  ...
  handleMessage(message) {
    ...
    if (type === 'loop-cache-data') {
      this.configureLoopCache(message);
      return;
    }
  ...
  resetTracks() {
    ...
    this.loopStartSample = 0;
    this.loopEndSample = 0;
    this.loopCacheByTrackIndex.clear();
    this.readingBlocked = true;
    this.fadeInFramesRemaining = 0;
    this.loopJumpCrossfadeBlocked = true;
    this.telemetryBaseSeconds = 0;
  ...
  configureTrack(config) {
    ...
    if (!track) {
      ...
      track.loopCache = this.loopCacheByTrackIndex.get(track.trackIndex) || null;
  ...
  configureLoopCache(message) {
    this.loopCacheByTrackIndex.clear();

    if (message.enabled !== true || !Array.isArray(message.tracks)) {
      for (let trackIndex = 0; trackIndex < this.trackCount; trackIndex += 1) {
        const track = this.tracks[trackIndex];
        if (track) {
          track.loopCache = null;
    ...
    for (let index = 0; index < message.tracks.length; index += 1) {
      const entry = message.tracks[index];
      const trackIndex = Math.max(0, Math.floor(Number(entry.trackIndex) || 0));
      if (!(entry.sampleBuffer instanceof SharedArrayBuffer) || !(entry.stateBuffer instanceof SharedArrayBuffer)) {
        continue;
      }

      const cache = {
        trackIndex,
        frameCount: Math.max(0, Math.floor(Number(entry.frameCount) || 0)),
        sampleRate: Number(entry.sampleRate) > 0 ? Number(entry.sampleRate) : (sampleRate || 48000),
        samples: new Int16Array(entry.sampleBuffer),
        state: new Int32Array(entry.stateBuffer),
      };
      this.loopCacheByTrackIndex.set(trackIndex, cache);
      const track = this.tracks[trackIndex];
      if (track) {
        track.loopCache = cache;
      }
    }
  ...
  applyLoopToTrack(track) {
    ...
    const loopCache = this.loopCacheByTrackIndex.get(track.trackIndex) || track.loopCache || null;
    if (!loopCache || loopCache.frameCount <= 0 || Atomics.load(loopCache.state, 0) !== 1) {
      track.loopEnabled = false;
      track.loopCache = null;
      return;
    }
  ...
  process(inputs, outputs) {
    ...
    try {
      for (let trackIndex = 0; trackIndex < this.trackCount; trackIndex += 1) {
        ...
        if (track.loopEnabled && track.loopCache) {
          this.mixPinnedLoopTrack(track, output, outputChannelCount, frameCount, renderStartFrame);
          continue;
        }
  ...
  getMasterPositionSeconds(frameOffset) {
    ...
    const baseFrame = Math.max(0, Math.round(this.telemetryBaseSeconds * rate));
    const linearFrame = Math.max(0, baseFrame + elapsedFrames);
    if (!this.loopEnabled || this.loopEndSample <= this.loopStartSample + 1) {
      return linearFrame / rate;
    }

    if (linearFrame < this.loopStartSample) {
      return linearFrame / rate;
    }

    const loopFrameCount = this.loopEndSample - this.loopStartSample;
    const wrappedFrame =
      this.loopStartSample + ((linearFrame - this.loopStartSample) % loopFrameCount);
    return wrappedFrame / rate;
  }
  ...
  mixPinnedLoopTrack(track, output, outputChannelCount, frameCount, renderStartFrame) {
    const cache = track.loopCache;
    if (
      !cache ||
      cache.frameCount <= 0 ||
      !cache.samples ||
      cache.samples.length < cache.frameCount ||
      Atomics.load(cache.state, 0) !== 1
    ) {
    ...
    const engineRate = sampleRate || 48000;
    const baseFrame = Math.max(0, Math.round(this.telemetryBaseSeconds * engineRate));
    const elapsedFrames = Math.max(0, renderStartFrame - this.telemetryBaseRenderedFrames);
    let cacheIndex = baseFrame + elapsedFrames - this.loopStartSample;
    cacheIndex %= cache.frameCount;
    if (cacheIndex < 0) {
      cacheIndex += cache.frameCount;
    }
    ...
    for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
      const rawSample = (cache.samples[cacheIndex] || 0) / 32767;

      track.effectiveGain += (targetGain - track.effectiveGain) * SOLO_GAIN_SMOOTHING;
      const mixedSample = rawSample * track.effectiveGain;
      const absoluteSample = mixedSample >= 0 ? mixedSample : -mixedSample;
      if (absoluteSample > peakSample) {
        peakSample = absoluteSample;
      }

      if (outputChannelCount === 1) {
        output[0][frameIndex] += mixedSample;
      } else if (outputChannelCount === 2) {
        if (outputRoute === 'right') {
          output[1][frameIndex] += mixedSample;
        ...
        for (let channelIndex = 0; channelIndex < outputChannelCount; channelIndex += 1) {
          output[channelIndex][frameIndex] += mixedSample;
        }
      }

      cacheIndex += 1;
      if (cacheIndex >= cache.frameCount) {
        cacheIndex = 0;
      }
    }

    track.absoluteReadFrame = Math.round(
      this.getMasterPositionSeconds(
        renderStartFrame - this.renderedFrames + frameCount,
      ) * track.sampleRate,
    );
