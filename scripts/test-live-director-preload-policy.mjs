...
assert.equal(
  ...
  false,
  'Chrome Android no debe descargar stems completos en segundo plano.',
);
...
assert.equal(
  ...
  false,
  'Los navegadores iPhone distintos de Safari tambien quedan protegidos.',
);
...
assert.equal(
  ...
  false,
  'iPadOS con agente de escritorio no debe confundirse con macOS.',
);
...
assert.equal(
  ...
  true,
  'El runtime nativo conserva el comportamiento de precarga existente.',
);
...
assert.match(
  directorSource,
  /shouldRunLiveDirectorFullFileWebPrewarm\(\{/,
  'Modo Ensayo debe aplicar la politica antes del prewarm Cache API.',
);
assert.match(
  directorSource,
  /isSafariWebBrowser\(\)/,
  'La exclusion Safari existente debe conservarse.',
);
assert.match(
  directorSource,
  /NativeLiveDirectorEngine\.preloadTracks\(\{/,
  'La ruta de precarga nativa debe permanecer activa.',
);
